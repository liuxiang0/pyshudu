...
class Sudoku:
    ...
    def __init__(self, width, height=None, board=None, difficulty=-1, seed=randrange(sys.maxsize)):
        ...
        assert self.width > 0,  '宽度不小于 1'
        assert self.height > 0, '高度也不小于 1'
        assert self.size > 1,   '指定数阵不能是 1 x 1'
        ...
        if board:
            ...
            random_seed(seed)
            shuffle(positions)
            # 仅仅提供了 9 个数，难度系数为 0.89 （8/9为空白)，且肯定有解。
    ...
    def difficulty(self, difficulty):
        assert 0 < difficulty < 1, '难度系数在0和1之间'
        indices = list(range(self.size * self.size))  # 9*9=81个数
        shuffle(indices)
        problem_board = self.solve().board  # 先求解，再隐藏部分解
        for index in indices[:int(difficulty * self.size * self.size)]:
            row_index = index // self.size
            col_index = index % self.size
            problem_board[row_index][col_index] = Sudoku._empty_cell_value  #故意隐藏解
    ...
    def show(self):
        ...
        if self.__difficulty == -1:
            print('无效数独. 请求解数独 (puzzle.solve()), 或设置数独难度 (puzzle.difficulty())')
            return self
        if not self.board:
            print('该模板无解')
            return self
